...
while True:
    ...
    while Networking_activity.upper() not in {'Y', 'N'}:
        print("Invalid input. Please enter Y or N.")
        Networking_activity = input("Any Networking today? Y or N? ")
    
    # Generate an ID if at least one activity is marked as 'Y'
    activity_id = f"DataScience{activity_id_counter:03}" if any(a.upper() == 'Y' for a in [R_activity, SQL_activity, Python_activity, Github_activity, Kaggle_activity, Terminal_activity, Excel_activity, Tech_reading_activity, Learning_activity, Data_analytics_activity, Tech_listings_activity, Projects_activity, Networking_activity]) else None
